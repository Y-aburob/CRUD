...
import { createUserByAxios } from '../api/axiosMethod/createUserByAxios';
import { deleteUserByAxios } from '../api/axiosMethod/deleteUserByAxios';
import { getUsersByAxios } from '../api/axiosMethod/getUsersByAxios';
import { ButtonComponent } from '../components';
import DialogComponent from '../components/DialogComponent';
import { editUserByAxios } from '../api/axiosMethod/editUserByAxios';
...
function AxiosCRUD() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const usersData = await getUsersByAxios(endPoint);
        setUsers(usersData);
      } catch (error) {
  ...
  const handleCreateUser = async () => {
    const newUser = await createUserByAxios(endPoint, {
      name: useForm.name,
      email: useForm.email,
  ...
  const saveUser = async () => {
    if (editingUser?.id) {
      const updatedUser = await editUserByAxios(endPoint, editingUser.id, {
        name: useForm.name,
        email: useForm.email,
        number: useForm.number,
        image: useForm.image,
  ...
  const handleDeleteUser = async () => {
    const usersAfterDelete = await deleteUserByAxios(endPoint, pickedUserId);
