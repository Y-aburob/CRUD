...
import { useCreateUser } from '../api/queryMethod/createUserQuery';
import { useDeleteUser } from '../api/queryMethod/deleteUserQuery';
import { useEditUser } from '../api/queryMethod/editUserQuery';
import { useGetUsersQuery } from '../api/queryMethod/useGetUsersQuery';
...
function QueryCRUD() {
  ...
  const endPoint = 'https://67bd8287321b883e790cc170.mockapi.io/axiosUsers';
  const [_editButton, setEditButton] = useState('Add User');
  const { data, isLoading } = useGetUsersQuery(endPoint);
  const { mutate } = useCreateUser(endPoint);
  const { mutate: deleteUser } = useDeleteUser(endPoint);
  ...
  const saveUser = async () => {
    if (editingUser?.id) {
      updateUser({ endPoint, userId: editingUser.id, user: useForm });
    } else {
      setEditButton('add user');
      handleCreateUser();
    }
  ...
  const handleDeleteUser = () => {
    deleteUser({ userId: pickedUserId });
    setShowDeleteMessage(false);
  };
    ...
    <Box>
      <Box marginX={3} marginTop={4}>
        ...
        <Box sx={{ marginTop: 10, overflow: 'auto', marginBottom: 1 }}>
          ...
          >
            A CRUD System using Query method
          </Typography>
          <Box
            display={'flex'}
            marginTop={5}
